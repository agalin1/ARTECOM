		...
		app.controller('usuario', function($scope,$http) {
			...
			$scope.message = 'Administracion Usuarios';
		    ...
		    $scope.get = function(id){
		    	...
		    	if(id=="") {
		    		$http.get("http://localhost/api/api/clientes").then(function (response) {
				        $scope.lista = response.data.data;
				        Materialize.toast(response.data.statusMessage, 100);
				...
				// Si la Id no esta en blanco, la solicitud se hace a un elemento especifico
		    		...
		    		$http.get("http://localhost/api/api/clientes/" + id).then(function (response) {
		    ...
		    $scope.post = function() {
		    	$http.post("http://localhost/api/api/clientes", $scope.nuevo)
		    ...
		    $scope.put = function(id) {
		    	...
		    	$http.put("http://localhost/api/api/clientes/" + id, $scope.nuevo)
			    	.then(
			    		function (response){
			            Materialize.toast(response.data.statusMessage, 4000);
			            $scope.nuevo = null;
		    ...
		    $scope.delete = function(id) {
		    	$http.delete("http://localhost/api/api/clientes/" + id)
				   .then(
				       function (response){
